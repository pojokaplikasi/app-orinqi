...
try {
  const legacyDataPath = path.join(
    ...
    "master_filie",
    "BAZI 01",
    "corrected_final_jieqi_data_1900_2150.json"
  )
  const legacyData = fs.readFileSync(legacyDataPath, "utf8")
  LEGACY_LICHUN_DATA = JSON.parse(legacyData)
} catch (e) {
  console.warn(
    "⚠️ Warning: corrected_final_jieqi_data_1900_2150.json not found"
  )
}
...
try {
  const completeDataPath = path.join(
    ...
    "master_filie",
    "BAZI 01",
    "jieqi_ultra_precise_1909_2183_cst.json"
  )
  ...
  console.warn(
    "⚠️ Warning: jieqi_ultra_precise_1909_2183_cst.json not found, using legacy data"
  )
}
...
export const HIDDEN_STEMS: Record<
  ...
  {
    main_qi: number | null
    sub_main_qi: number | null
    residual_qi: number | null
  }
> = {
...
export function getChineseNewYearBoundary(
  ...
  if (
    LEGACY_LICHUN_DATA &&
    LEGACY_LICHUN_DATA[year.toString()] &&
...
export function getHiddenStemsWithTenGods(
  ...
  const hiddenData = HIDDEN_STEMS[branchName] || {
    main_qi: null,
    sub_main_qi: null,
    residual_qi: null,
  }
...
export function calculateLuckPillars(
  ...
  const startStemIdx = HEAVENLY_STEMS.findIndex(
    (stem) => stem.name === fourPillars.month_pillar.heavenly_stem.name
  )
  const startBranchIdx = EARTHLY_BRANCHES.findIndex(
    (branch) => branch.name === fourPillars.month_pillar.earthly_branch.name
  )
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const luckPillar = {
      ...
      gan_zhi:
        GANZHI_COMBINATIONS[(stemIdx * 6 + Math.floor(branchIdx / 2)) % 60],
...
export function calculateYearlyPillars(
  startYear: number,
  endYear: number,
  birthTime: dayjs.Dayjs
): any[] {
...
export function calculateDailyPillars(
  year: number,
  month: number,
  birthTime: dayjs.Dayjs
): any[] {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dailyPillar = {
      ...
      hidden_stems: getHiddenStemsWithTenGods(
        dayBranchIndex,
        birthDayStemIndex
      ),
      gan_zhi:
...
export function calculateHourlyPillars(
  year: number,
  month: number,
  day: number,
  birthTime: dayjs.Dayjs
