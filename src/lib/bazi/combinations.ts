...
// XING (刑) / PUNISHMENTS
// ============================================
export const UNGRATEFUL_PUNISHMENT = [
  { branches: [2, 5, 8], name: "Ungrateful Punishment", icon: "⚠️" },
]
export const ARROGANT_PUNISHMENT = [
  { branches: [7, 10, 1], name: "Arrogant Punishment", icon: "⚠️" },
]
export const RUDE_PUNISHMENT = [
  { pair: [0, 3], name: "Rude Punishment", icon: "⚠️" },
]
...
export function canFormSeasonalUnion(
  branch1Index: number,
  branch2Index: number
) {
...
export function canFormThreeHarmony(
  branch1Index: number,
  branch2Index: number
) {
...
export function canFormHalfCombination(
  branch1Index: number,
  branch2Index: number
) {
...
export function canFormUngratefulPunishment(
  branch1Index: number,
  branch2Index: number
) {
...
export function canFormArrogantPunishment(
  branch1Index: number,
  branch2Index: number
) {
...
export function canFormRudePunishment(
  branch1Index: number,
  branch2Index: number
) {
...
export function canFormSelfPunishment(
  branch1Index: number,
  branch2Index: number
) {
...
// ============================================
export function detectLuckPillarCombinations(
  luckPillar: any,
  fourPillars: any
): { hsCombos: any[]; branchInteractions: any[] } {
  const natalPillars = [
    {
      name: "H",
  ...
  const luckStemIndex = HEAVENLY_STEMS.findIndex(
    (s) => s.name === luckPillar.heavenly_stem.name
  )
  const luckBranchIndex = EARTHLY_BRANCHES.findIndex(
    (b) => b.name === luckPillar.earthly_branch.name
  )

  natalPillars.forEach((natal) => {
    // HS Combinations
    const natalStemIndex = HEAVENLY_STEMS.findIndex(
      (s) => s.name === natal.stem.name
    )
...
export function detectAllBranchInteractions(
  fourPillars: any,
  currentPillars: any
) {
  const allPillars = [
