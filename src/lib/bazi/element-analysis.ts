import { HEAVENLY_STEMS, EARTHLY_BRANCHES, HIDDEN_STEMS_MAP } from './constants';

export function getTenGodsRelationship(dayMasterIndex: number, stemIndex: number): string {
...
export function calculateElementStructure(fourPillars: any, currentPillars: any) {
    ...
    if (currentPillars && currentPillars.current_luck) calculatePillar(currentPillars.current_luck, false);
    if (currentPillars && currentPillars.current_year) calculatePillar(currentPillars.current_year, false);
    if (currentPillars && currentPillars.current_month) calculatePillar(currentPillars.current_month, false);
    if (currentPillars && currentPillars.current_day) calculatePillar(currentPillars.current_day, false);
    ...
    Object.keys(elements).forEach(elem => {
        natalPercentages[elem] = natalTotal > 0 ? (elements[elem].natal / natalTotal * 100).toFixed(1) : "0.0";
        annualPercentages[elem] = annualTotal > 0 ? (elements[elem].annual / annualTotal * 100).toFixed(1) : "0.0";
    });
...
export function calculateTenGods(fourPillarsData: any, currentPillars: any) {
    ...
    const tenGodsNames = [
        ...
        'Direct Wealth', 'Indirect Wealth', 'Direct Officer', 
        'Seven Killings', 'Direct Resource', 'Indirect Resource'
    ];
    ...
    const getTenGodFromStem = (stemIndex: number) => {
        ...
        if (element === dayMasterElement) return yinYang === dayMasterYinYang ? 'Friend' : 'Rob Wealth';
        if ((dayMasterElement + 1) % 5 === element) return yinYang === dayMasterYinYang ? 'Eating God' : 'Hurting Officer';
        if ((dayMasterElement + 2) % 5 === element) return yinYang === dayMasterYinYang ? 'Indirect Wealth' : 'Direct Wealth';
        if ((dayMasterElement + 3) % 5 === element) return yinYang === dayMasterYinYang ? 'Seven Killings' : 'Direct Officer';
        if ((dayMasterElement + 4) % 5 === element) return yinYang === dayMasterYinYang ? 'Direct Resource' : 'Indirect Resource';
    ...
    const processPillar = (pillar: any, targetType: string) => {
        ...
        if (pillar.hidden_stems) {
            ...
            if (isMaoYouZi) {
                mainQiWeight = 1.0; subMainQiWeight = 0; residualQiWeight = 0;
            } else if (isHaiOrWu) {
                mainQiWeight = 0.8; subMainQiWeight = 0.2; residualQiWeight = 0;
            } else {
                mainQiWeight = 0.7; subMainQiWeight = 0.2; residualQiWeight = 0.1;
            }
