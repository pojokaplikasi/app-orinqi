...
import { getHiddenStems, getHiddenStemsWithTenGods } from "./element-analysis"
...
export function getNayinFromStemBranch(
  stemName: string,
  branchName: string
): string {
...
export function getLifeCycleIndex(
  stemName: string,
  branchName: string
): number {
  const pinyinStem = stemToPinyin(stemName)
...
export function formatLifeCycleName(
  stemName: string,
  branchName: string,
  ...
  return style === "classic"
    ? LIFE_CYCLES_PINYIN[index]
    : LIFE_CYCLES_ENGLISH[index]
}

export function formatNayinName(
  name: string,
  style: "classic" | "modern" = "modern"
): string {
...
export function calculateCurrentYearPillar(
  currentDate: Date,
  fourPillarsData: any
) {
...
export function calculateCurrentMonthPillar(
  currentDate: Date,
  fourPillarsData: any
) {
...
export function calculateCurrentDayPillar(
  currentDate: Date,
  fourPillarsData: any
) {
  const dayForCalculation = new Date(currentDate)
  ...
  const daysSinceRef = Math.floor(
    (dayForCalculation.getTime() - refDate.getTime()) / (1000 * 60 * 60 * 24)
  )

  const oct20_1987 = new Date(1987, 9, 20)
  const daysToOct1987 = Math.floor(
    (oct20_1987.getTime() - refDate.getTime()) / (1000 * 60 * 60 * 24)
  )
...
export function calculateCurrentLuckPillar(
  currentDate: Date,
  birthTimeData: any,
  fourPillarsData: any,
  luckPillarsData: any
