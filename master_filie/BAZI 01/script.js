...
const NAYIN_TABLE = {
  "Jia-Zi": "Sea metal",
  "Yi-Chou": "Sea metal",
  "Bing-Yin": "Furnace fire",
  "Ding-Mao": "Furnace fire",
  "Wu-Chen": "Forest wood",
  "Ji-Si": "Forest wood",
  "Geng-Wu": "Road earth",
  "Xin-Wei": "Road earth",
  "Ren-Shen": "Sword metal",
  "Gui-You": "Sword metal",
  "Jia-Xu": "Volcanic fire",
  "Yi-Hai": "Volcanic fire",
  "Bing-Zi": "Cave water",
  "Ding-Chou": "Cave water",
  "Wu-Yin": "Fortress earth",
  "Ji-Mao": "Fortress earth",
  "Geng-Chen": "Wax metal",
  "Xin-Si": "Wax metal",
  "Ren-Wu": "Willow wood",
  "Gui-Wei": "Willow wood",
  "Jia-Shen": "Stream water",
  "Yi-You": "Stream water",
  "Bing-Xu": "Roof tiles earth",
  "Ding-Hai": "Roof tiles earth",
  "Wu-Zi": "Lightning fire",
  "Ji-Chou": "Lightning fire",
  "Geng-Yin": "Conifer wood",
  "Xin-Mao": "Conifer wood",
  "Ren-Chen": "River water",
  "Gui-Si": "River water",
  "Jia-Wu": "Sand metal",
  "Yi-Wei": "Sand metal",
  "Bing-Shen": "Forest fire",
  "Ding-You": "Forest fire",
  "Wu-Xu": "Meadow wood",
  "Ji-Hai": "Meadow wood",
  "Geng-Zi": "Adobe earth",
  "Xin-Chou": "Adobe earth",
  "Ren-Yin": "Precious metal",
  "Gui-Mao": "Precious metal",
  "Jia-Chen": "Lamp fire",
  "Yi-Si": "Lamp fire",
  "Bing-Wu": "Sky water",
  "Ding-Wei": "Sky water",
  "Wu-Shen": "Highway earth",
  "Ji-You": "Highway earth",
  "Geng-Xu": "Jewellery metal",
  "Xin-Hai": "Jewellery metal",
  "Ren-Zi": "Mulberry wood",
  "Gui-Chou": "Mulberry wood",
  "Jia-Yin": "Rapids water",
  "Yi-Mao": "Rapids water",
  "Bing-Chen": "Desert earth",
  "Ding-Si": "Desert earth",
  "Wu-Wu": "Sun fire",
  "Ji-Wei": "Sun fire",
  "Geng-Shen": "Pomegranate wood",
  "Xin-You": "Pomegranate wood",
  "Ren-Xu": "Ocean water",
  "Gui-Hai": "Ocean water",
}
...
function getNayinFromStemBranch(stemName, branchName) {
  ...
  console.log(
    `Nayin lookup: ${stemName} + ${branchName} -> ${pinyinStem}-${pinyinBranch} -> ${result}`
  )
  return result
}
...
function getLifeCycleIndex(stemName, branchName) {
  ...
  console.log(
    `LifeCycle lookup: ${stemName} + ${branchName} -> ${pinyinStem}-${pinyinBranch} -> index ${index}`
  )
...
function resetPillarSelection() {
  ...
  if (window.currentBaziData) {
    displayTimePeriodRows(
      window.currentBaziData.four_pillars,
      window.currentBaziData.luck_pillars
    )
    displayCurrentPillars(
      window.currentBaziData.four_pillars,
      window.currentBaziData.luck_pillars
    )
  }
...
function setCombinationStyle(style) {
  ...
  if (window.currentBaziData && window.currentBaziData.four_pillars) {
    ...
    detectAndDisplayHSCombinations(
      window.currentBaziData.four_pillars,
      window.currentBaziData.luck_pillars
    )
  }
...
function calculateLuckyStars(fourPillars, currentPillars) {
  ...
  const peachBlossomMap = {
    Shen: "You",
    Zi: "You",
    Chen: "You", // Monkey-Rat-Dragon → Rooster
    Hai: "Zi",
    Mao: "Zi",
    Wei: "Zi", // Pig-Rabbit-Goat → Rat
    Yin: "Mao",
    Wu: "Mao",
    Xu: "Mao", // Tiger-Horse-Dog → Rabbit
    Si: "Wu",
    You: "Wu",
    Chou: "Wu", // Snake-Rooster-Ox → Horse
  }
  ...
  const skyHorseMap = {
    Shen: "Yin",
    Zi: "Yin",
    Chen: "Yin", // Monkey-Rat-Dragon → Tiger
    Hai: "Shen",
    Mao: "Shen",
    Wei: "Shen", // Pig-Rabbit-Goat → Monkey
    Yin: "Shen",
    Wu: "Shen",
    Xu: "Shen", // Tiger-Horse-Dog → Monkey
    Si: "Hai",
    You: "Hai",
    Chou: "Hai", // Snake-Rooster-Ox → Pig
  }
  ...
  const solitaryMap = {
    Hai: "Yin",
    Zi: "Yin",
    Chou: "Yin", // Water → Tiger
    Yin: "Si",
    Mao: "Si",
    Chen: "Si", // Wood → Snake
    Si: "Shen",
    Wu: "Shen",
    Wei: "Shen", // Fire → Monkey
    Shen: "Hai",
    You: "Hai",
    Xu: "Hai", // Metal → Pig
  }
  ...
  const dayBranchIndex = EARTHLY_BRANCHES.findIndex(
    (b) => b.name === dayBranchEnglish
  )
  ...
  if (currentPillars) {
    if (currentPillars.current_luck)
      allPillars.push(currentPillars.current_luck)
    if (currentPillars.current_year)
      allPillars.push(currentPillars.current_year)
    if (currentPillars.current_month)
      allPillars.push(currentPillars.current_month)
  ...
  if (stars.peachBlossom && !branchToPillars[stars.peachBlossom]) {
    console.log(
      "WARNING: Peach Blossom",
      stars.peachBlossom,
      "not found in any pillar"
    )
  }
  ...
  if (stars.heavenlyDoctor && !branchToPillars[stars.heavenlyDoctor]) {
    console.log(
      "WARNING: Heavenly Doctor",
      stars.heavenlyDoctor,
      "not found in any pillar"
    )
  }
...
function displayLuckyStars(stars) {
  ...
  const getBranchInfo = (branchTraditionalName) => {
    ...
    return branch
      ? { character: branch.character, english: branch.name }
      : { character: branchTraditionalName, english: englishName }
  }
  ...
  // 1. Nobleman Star
  const noblemanBranches =
    stars.nobleman.length > 0
      ? stars.nobleman
          .map((b) => {
  ...
  // 2. Intelligence Star
  const intelInfo = stars.intelligence
    ? getBranchInfo(stars.intelligence)
    : { character: "", english: "" }
  html += `
        <tr>
  ...
  const peachInfo = stars.peachBlossom
    ? getBranchInfo(stars.peachBlossom)
    : { character: "", english: "" }
  html += `
        <tr>
  ...
  // 4. Sky Horse Star
  const skyInfo = stars.skyHorse
    ? getBranchInfo(stars.skyHorse)
    : { character: "", english: "" }
  html += `
        <tr>
  ...
  // 5. Solitary Star
  const solitaryInfo = stars.solitary
    ? getBranchInfo(stars.solitary)
    : { character: "", english: "" }
  html += `
        <tr>
  ...
  const doctorInfo = stars.heavenlyDoctor
    ? getBranchInfo(stars.heavenlyDoctor)
    : { character: "", english: "" }
  html += `
        <tr>
  ...
  const kongwangLabel =
    window.currentMode === "modern" ? "DEAD EMPTINESS 空亡" : "KONG WANG 空亡"
...
function displayElementStructure(elementData) {
  ...
  if (
    window.elementRadarChart &&
    typeof window.elementRadarChart.destroy === "function"
  ) {
  ...
  fetch("/calculate", {
    ...
    .then((data) => {
      ...
      if (birthTimeData && birthTimeData.unknownBirthTime) {
        ...
        console.log(
          "Hour Pillar cleared completely - birth time unknown, no combinations"
        )
      } else {
...
function displayPillar(pillarData, element) {
  ...
  if (pillarData.heavenly_stem && window.currentBaziData) {
    ...
    if (stemIndex >= 0) {
      // Get Day Master info
      const dayMasterName =
        window.currentBaziData.four_pillars.day_pillar.heavenly_stem.name
      const dayMasterIndex = HEAVENLY_STEMS.findIndex(
        (s) => s.name === dayMasterName
      )
      const dayMasterElement = Math.floor(dayMasterIndex / 2)
      ...
      // Output (Day Master produces)
        ...
        tenGodName =
          yinYang === dayMasterYinYang ? "Eating God" : "Hurting Officer"
      }
      // Wealth (Day Master controls)
      else if ((dayMasterElement + 2) % 5 === element) {
        tenGodName =
          yinYang === dayMasterYinYang ? "Indirect Wealth" : "Direct Wealth"
      }
      // Officer (Controls Day Master)
      else if ((dayMasterElement + 3) % 5 === element) {
        tenGodName =
          yinYang === dayMasterYinYang ? "Seven Killings" : "Direct Officer"
      }
      // Resource (Produces Day Master)
      else if ((dayMasterElement + 4) % 5 === element) {
        tenGodName =
          yinYang === dayMasterYinYang ? "Direct Resource" : "Indirect Resource"
      }
  ...
  if (pillarData.earthly_branch && pillarData.earthly_branch.name) {
    ...
    if (window.currentLuckyStars) {
      ...
      if (
        Array.isArray(window.currentLuckyStars.nobleman) &&
        window.currentLuckyStars.nobleman.includes(branchTraditional)
      ) {
        console.log(
          `✓ Nobleman: ${branchTraditional} found in`,
          window.currentLuckyStars.nobleman
        )
        starsForThisBranch.push("👑")
      }
      // Others are strings - check exact match
      if (
        window.currentLuckyStars.intelligence &&
        window.currentLuckyStars.intelligence === branchTraditional
      ) {
        console.log(`✓ Intelligence: ${branchTraditional} matches`)
        starsForThisBranch.push("🎓")
      }
      if (
        window.currentLuckyStars.peachBlossom &&
        window.currentLuckyStars.peachBlossom === branchTraditional
      ) {
        console.log(`✓ Peach Blossom: ${branchTraditional} matches`)
        starsForThisBranch.push("🌸")
      }
      if (
        window.currentLuckyStars.skyHorse &&
        window.currentLuckyStars.skyHorse === branchTraditional
      ) {
        console.log(`✓ Sky Horse: ${branchTraditional} matches`)
        starsForThisBranch.push("🦄")
      }
      if (
        window.currentLuckyStars.solitary &&
        window.currentLuckyStars.solitary === branchTraditional
      ) {
        console.log(`✓ Solitary: ${branchTraditional} matches`)
        starsForThisBranch.push("🌙")
      }
      if (
        window.currentLuckyStars.heavenlyDoctor &&
        window.currentLuckyStars.heavenlyDoctor === branchTraditional
      ) {
      ...
      if (
        Array.isArray(window.currentLuckyStars.kongwang) &&
        window.currentLuckyStars.kongwang.includes(branchTraditional)
      ) {
        console.log(
          `✓ Kong Wang (Dead Emptiness): ${branchTraditional} matches`,
          window.currentLuckyStars.kongwang
        )
    ...
    if (starsForThisBranch.length > 0) {
      ...
      const stackedStars = starsForThisBranch
        .map(
          (star) =>
      ...
      console.log(
        `✅ Indicator CREATED for ${branchTraditional}: ${starsForThisBranch.join(", ")}`
      )
    } else {
...
function calculateCurrentLuckPillar(
  ...
  if (
    ...
    const nayinName = getNayinFromStemBranch(
...
function displayLuckPillar(pillarData, div) {
  ...
  const nayinName = formatNayinName(
    getNayinFromStemBranch(heavenlyStemName, earthlyBranchName)
  )
  const nayinElement = pillarData.gan_zhi
...
function enrichLuckPillarForCurrent(pillar) {
  ...
  return addCurrentPeriodMetadata(
    ...
    "Period:",
    `${pillar.year_start}-${pillar.year_end}`
  )
}

function getActiveCurrentContext(luckPillarsData) {
  const realDate = new Date()
  const activeLuckPillar =
    selectedPillars.luck !== null
      ? luckPillarsData.luck_pillars[selectedPillars.luck]
      : null
  ...
  if (year === null) {
    if (activeLuckPillar) {
      ...
      year =
        realYear >= activeLuckPillar.year_start &&
        realYear <= activeLuckPillar.year_end
          ? realYear
  ...
  const month =
    selectedPillars.month !== null
      ? selectedPillars.month
      : realDate.getMonth() + 1
  const maxDay = getDaysInMonth(year, month)
  const day =
    selectedPillars.day !== null
      ? Math.min(selectedPillars.day, maxDay)
      : Math.min(realDate.getDate(), maxDay)
...
function getActiveCurrentPillars(fourPillarsData, luckPillarsData) {
  ...
  const currentLuckPillar =
    selectedPillars.luck !== null
      ? enrichLuckPillarForCurrent(
          luckPillarsData.luck_pillars[selectedPillars.luck]
        )
      : (() => {
          const pillar = calculateCurrentLuckPillar(
            new Date(),
            fourPillarsData,
            luckPillarsData
          )
          return pillar.luck_period
            ? addCurrentPeriodMetadata(pillar, "Period:", pillar.luck_period)
            : pillar
        })()

  const currentYearPillar = addCurrentPeriodMetadata(
    selectedPillars.yearPillar ||
      calculateCurrentYearPillar(context.activeDate, fourPillarsData),
    "Year:",
    String(context.year)
  )

  const currentMonthPillar = addCurrentPeriodMetadata(
    selectedPillars.monthPillar ||
      calculateCurrentMonthPillar(context.activeDate, fourPillarsData),
    "Month:",
    `${monthNamesFull[context.month - 1]} ${context.year}`
  )

  const currentDayPillar = addCurrentPeriodMetadata(
    selectedPillars.dayPillar ||
      calculateCurrentDayPillar(context.activeDate, fourPillarsData),
    "Date:",
...
function displayCurrentPillars(fourPillarsData, luckPillarsData) {
  const activeCurrentPillars = getActiveCurrentPillars(
    fourPillarsData,
    luckPillarsData
  )
  ...
  updateElementComposition(
    fourPillarsData,
    luckPillarsData,
    activeCurrentPillars
  )
}

// Update Element Composition with current pillars
function updateElementComposition(
  fourPillarsData,
  luckPillarsData,
  activeCurrentPillars = null
) {
  const currentPillars =
    activeCurrentPillars ||
    getActiveCurrentPillars(fourPillarsData, luckPillarsData)
...
function displayTenGods(tenGodsData) {
  ...
  sortedGods.forEach(([godName, natalPoints]) => {
    ...
    const natalStems =
      natalStemsSet.size > 0 ? Array.from(natalStemsSet).join(" ") : "" // Empty if no natal stems
    const annualStems =
      annualStemsSet.size > 0 ? Array.from(annualStemsSet).join(" ") : "" // Empty if no annual stems

    // DEBUG: Log each 10 God
    console.log(
      `10 God: ${godName} | Natal: ${natalPoints} (${natalStems || "none"}) | Annual: ${annualPoints} (${annualStems || "none"}) | Combined: ${allStems}`
    )

    // Calculate percentages
    const natalPercent =
      totalNatal > 0 ? ((natalPoints / totalNatal) * 100).toFixed(1) : "0.0"
    const annualPercent =
      totalAnnual > 0 ? ((annualPoints / totalAnnual) * 100).toFixed(1) : "0.0"
...
function updateDayPillarSelection() {
  ...
  allPillars.forEach((pillarDiv, index) => {
    const pillarTitle =
      pillarDiv.querySelector(".time-period-title")?.textContent
...
function createTimePeriodPillar(pillarData, title) {
  ...
  const ganZhiName = formatNayinName(
    getNayinFromStemBranch(
      pillarData.heavenly_stem.name,
      pillarData.earthly_branch.name
    )
  )
  const ganZhiElement = pillarData.gan_zhi
    ? pillarData.gan_zhi.element_name
    : ""
  const lifeCycleName = formatLifeCycleName(
    pillarData.heavenly_stem.name,
    pillarData.earthly_branch.name
  )
  ...
  let luckyStarsIndicator = ""
  if (
    pillarData.earthly_branch &&
    pillarData.earthly_branch.name &&
    window.currentLuckyStars
  ) {
    const branchEnglishName = pillarData.earthly_branch.name
    const branchNameMap = {
      Rat: "Zi",
...
function canFormAnhe(branch1Index, branch2Index) {
  for (const anhe of ANHE_PAIRS) {
    if (
      (anhe.pair[0] === branch1Index && anhe.pair[1] === branch2Index) ||
