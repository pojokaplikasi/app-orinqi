#!/usr/bin/env python3
"""
Comprehensive Lichun Verification for ALL 275 years (1909-2183)
Tests every single year against Excel data
"""
...
# Complete Excel Lichun data (WIB timezone)
excel_lichun_data = {
...
print("="*80)
print("COMPREHENSIVE LICHUN VERIFICATION (1909-2183)")
print("Testing ALL 275 years against Excel data")
print("="*80)
...
for year in range(1909, 2184):
    ...
    month, day, hour, minute = excel_data
    
    # Create Excel datetime (WIB)
    excel_lichun_wib = datetime(year, month, day, hour, minute)
    excel_lichun_str = excel_lichun_wib.strftime('%Y-%m-%d %H:%M')
    
    # Get generated data
    if str(year) not in jieqi_data:
        print(f"❌ Year {year}: Data NOT FOUND in JSON!")
        mismatch_list.append((year, excel_lichun_str, "NOT FOUND", 999))
        continue
    ...
    # Compare
    if generated_lichun_str == excel_lichun_str:
        match_count += 1
        status = "✅"
    else:
        diff_minutes = (generated_lichun_wib - excel_lichun_wib).total_seconds() / 60
        mismatch_list.append((year, excel_lichun_str, generated_lichun_str, diff_minutes))
        status = "❌"
    
    # Print progress (show mismatches immediately, and every 50 years)
    if status == "❌" or year % 50 == 0 or year == 2183:
        print(f"{status} Year {year}: Excel={excel_lichun_str} | Generated={generated_lichun_str}", end="")
        if status == "❌":
            print(f" [DIFF: {diff_minutes:.1f} min]")
        else:
            print(f" ✅ MATCH")
...
print(f"Mismatched: {len(mismatch_list)}")
print(f"Accuracy: {match_count/275*100:.2f}%")

if mismatch_list:
    print("\n" + "="*80)
    print("MISMATCH DETAILS (Requires Fix):")
    print("="*80)
    for year, excel_str, gen_str, diff in mismatch_list:
        print(f"❌ Year {year}: Excel={excel_str} | Generated={gen_str} [Diff: {diff:.1f} min]")
    
    print("\n⚠️  ATTENTION: Some years need correction!")
else:
    print("\n🎉 PERFECT! ALL 275 YEARS MATCH 100%!")
    print("="*80)
