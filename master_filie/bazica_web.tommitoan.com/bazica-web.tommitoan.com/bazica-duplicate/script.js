...
function displayPillar(pillarData, element) {
  ...
  // For Current Pillars, hide 12 Phrase but show combinations and period
  const isCurrentPillar = div.id && div.id.startsWith("Current")

  // 12 Phrase - HANYA untuk Natal Chart
  const lifecycleHTML = isCurrentPillar
    ? ""
    : `
        <hr>
        <div class="lifecycle-separator">
  ...
  let periodHTML = ""
  if (isCurrentPillar && window.currentBaziData) {
    const currentDate = new Date()

    if (div.id === "CurrentLuckPillar") {
      const birthDate = new Date(birthTimeData.dateTime)
      const currentAge = currentDate.getFullYear() - birthDate.getFullYear()
      const firstLuckPillar =
        window.currentBaziData.luck_pillars.luck_pillars[0]
      ...
      const validIndex = Math.max(
        ...
        Math.min(
          luckPillarIndex,
          window.currentBaziData.luck_pillars.luck_pillars.length - 1
        )
      )
      const luckPillar =
        window.currentBaziData.luck_pillars.luck_pillars[validIndex]
      if (luckPillar && luckPillar.year_start && luckPillar.year_end) {
        periodHTML = `
                    <div style="padding: 0.5rem; background: rgba(0,0,0,0.03); border-radius: 8px; text-align: center; margin-top: 0.5rem;">
                        <div style="font-size: 0.85rem; font-weight: 700; color: #666;">Period:</div>
                        <div style="font-size: 1.1rem; font-weight: 700; color: #333; margin-top: 0.2rem;">${luckPillar.year_start}-${luckPillar.year_end}</div>
                    </div>
                `
      }
    } else if (div.id === "CurrentYearPillar") {
      const year = currentDate.getFullYear()
      periodHTML = `
                <div style="padding: 0.5rem; background: rgba(0,0,0,0.03); border-radius: 8px; text-align: center; margin-top: 0.5rem;">
                    <div style="font-size: 0.85rem; font-weight: 700; color: #666;">Year:</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: #333; margin-top: 0.2rem;">${year}</div>
                </div>
            `
    } else if (div.id === "CurrentMonthPillar") {
      const monthNames = [
        "January",
        "February",
        "March",
      ...
      periodHTML = `
                <div style="padding: 0.5rem; background: rgba(0,0,0,0.03); border-radius: 8px; text-align: center; margin-top: 0.5rem;">
                    <div style="font-size: 0.85rem; font-weight: 700; color: #666;">Month:</div>
                    <div style="font-size: 1rem; font-weight: 700; color: #333; margin-top: 0.2rem;">${month} ${year}</div>
                </div>
            `
    } else if (div.id === "CurrentDayPillar") {
      ...
      periodHTML = `
                <div style="padding: 0.5rem; background: rgba(0,0,0,0.03); border-radius: 8px; text-align: center; margin-top: 0.5rem;">
                    <div style="font-size: 0.85rem; font-weight: 700; color: #666;">Date:</div>
                    <div style="font-size: 1rem; font-weight: 700; color: #333; margin-top: 0.2rem;">${day} ${month} ${year}</div>
                </div>
            `
    }
  }
...
function calculateCurrentLuckPillar(
  ...
  if (
    ...
    const phrase12 = getLifeCycleName((stemIndex + branchIndex) % 12)
...
function displayCurrentPillars(fourPillarsData, luckPillarsData) {
  ...
  const currentDayPillar = calculateCurrentDayPillar(
    currentDate,
    fourPillarsData
  )
  ...
  updateElementComposition(fourPillarsData, luckPillarsData)
}

// Update Element Composition with current pillars
function updateElementComposition(fourPillarsData, luckPillarsData) {
  const currentDate = new Date()
...
function getLifeCycleName(index) {
  if (combinationStyle === "classic") {
    return LIFE_CYCLES_PINYIN[index] || ""
  } else {
    return LIFE_CYCLES_ENGLISH[index] || ""
  }
}
