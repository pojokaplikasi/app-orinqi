...
function displayPillar(pillarData, element) {
    ...
    if (isCurrentPillar && window.currentBaziData) {
        ...
        if (div.id === 'CurrentLuckPillar') {
            ...
            const firstLuckPillar = window.currentBaziData.luck_pillars.luck_pillars[0];
            const baseAge = firstLuckPillar.year_start - birthDate.getFullYear();
            const luckPillarIndex = Math.floor((currentAge - baseAge) / 10);
            const validIndex = Math.max(0, Math.min(luckPillarIndex, window.currentBaziData.luck_pillars.luck_pillars.length - 1));
            const luckPillar = window.currentBaziData.luck_pillars.luck_pillars[validIndex];
...
function calculateCurrentLuckPillar(currentDate, fourPillarsData, luckPillarsData) {
    ...
    if (luckPillarIndex >= 0 && luckPillarIndex < luckPillarsData.luck_pillars.length) {
        ...
        const stemIndex = HEAVENLY_STEMS.findIndex(s => s.name === activeLuckPillar.heavenly_stem.name);
...
function displayLuckPillar(pillarData, div) {
    ...
    const branchIndex = EARTHLY_BRANCHES.findIndex(b => b.name === earthlyBranchName);
    const nayinIndex = (stemIndex * 6 + Math.floor(branchIndex / 2)) % 60;
    const nayinData = GANZHI_COMBINATIONS[nayinIndex];
...
function displayCurrentPillars(fourPillarsData, luckPillarsData) {
    ...
    const currentDate = new Date();
    const currentMonthPillar = calculateCurrentMonthPillar(currentDate, fourPillarsData);
    ...
    const currentYearPillar = calculateCurrentYearPillar(currentDate, fourPillarsData);
    displayPillar(currentYearPillar, "CurrentYearPillar");
    
    // Calculate current 10-year luck pillar
    const currentLuckPillar = calculateCurrentLuckPillar(currentDate, fourPillarsData, luckPillarsData);
